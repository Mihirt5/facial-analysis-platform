...
import { mediaPipeClientService, type FacialAnalysis } from '~/lib/mediapipe-client';
import { ANALYSIS_STRUCTURE } from '~/lib/analysis-structure';

interface TestAnalysisProcessorProps {
  analysisId: string;
  images: {
    front: string;
    left: string;
...
export function TestAnalysisProcessor({ 
  analysisId, 
  images, 
  onComplete 
}: TestAnalysisProcessorProps) {
  ...
  const getSectionValue = (sectionKey: string, analysis: FacialAnalysis): number => {
    const valueMap: Record<string, number> = {
      // Eye Area
      'eye_area_canthal_tilt': analysis.eyeArea.canthalTilt,
      'eye_area_upper_eyelid_exposure': analysis.eyeArea.upperEyelidExposure,
      'eye_area_spacing_symmetry': analysis.eyeArea.spacingSymmetry,
      'eye_area_periorbital_support': analysis.eyeArea.periorbitalSupport,
      
      // Nose
      'nose_dorsal_profile': analysis.nose.dorsalProfile,
      'nose_tip_rotation': analysis.nose.tipRotation,
      'nose_nostril_exposure': analysis.nose.nostrilExposure,
      'nose_skin_thickness': analysis.nose.skinThickness,
      
      // Jaw
      'jaw_chin_projection': analysis.jaw.chinProjection,
      'jaw_gonial_angle': analysis.jaw.gonialAngle,
      'jaw_jawline_definition': analysis.jaw.jawlineDefinition,
      'jaw_lower_third_prominence': analysis.jaw.lowerThirdProminence,
      
      // Skin
      'skin_elasticity': analysis.skin.elasticity,
      'skin_damage': analysis.skin.damage,
      'skin_hydration_texture': analysis.skin.hydrationTexture,
      'skin_fine_lines': analysis.skin.fineLines
    };

    return valueMap[sectionKey] || 0.5; // Default to middle value if not found
  };

  const processAnalysis = async () => {
    setIsProcessing(true);
    setError(null);
    setCurrentStep('Initializing MediaPipe...');

    try {
      console.log('Starting analysis with images:', images);
      
      // Initialize MediaPipe
      await mediaPipeClientService.initialize();
      setCurrentStep('Processing facial landmarks...');

      // Process images with MediaPipe
      const analysis = await mediaPipeClientService.processAnalysis(images);
      setCurrentStep('Generating analysis content...');
      ...
      for (const subtab of ANALYSIS_STRUCTURE) {
        for (const section of subtab.sections) {
          const value = getSectionValue(section.key, analysis);
          const explanation = generateExplanation(section.key, value);
          
          content.push({
            sectionKey: section.key,
            image: images.front, // Use front image for all sections for now
            explanation
          });
        }

        // Generate additional features for each subtab
        const additionalText = generateAdditionalFeatures(subtab.key, analysis);
        additionalFeatures.push({
          sectionKey: subtab.key,
          text: additionalText
        });
      }
      ...
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-green-700">
            Your facial analysis has been processed successfully with MediaPipe.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-lg shadow">
            <h4 className="font-semibold mb-4">Raw Analysis Data</h4>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h4 className="font-semibold mb-4">Generated Content</h4>
            <div className="space-y-2 text-sm">
              <p><strong>Sections:</strong> {results.content.length}</p>
              <p><strong>Additional Features:</strong> {results.additionalFeatures.length}</p>
              <p><strong>Sample Explanation:</strong></p>
              <p className="text-gray-600 italic">
                {results.content[0]?.explanation?.substring(0, 100)}...
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-4 p-8">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Click below to start processing your facial analysis with MediaPipe
        </p>
        <button 
